...
class Engine:
    def get_engine(self,SCHEMA):    
        ...
        USER = os.environ.get('SNOWFLAKE_USER_SP')
        PASSWORD = os.environ.get('SNOWFLAKE_PASSWORD_SP')
        DB = os.environ.get('SNOWFLAKE_DATABASE_SP')
        WH = os.environ.get('SNOWFLAKE_WAREHOUSE_SP')
        ROLE = os.environ.get('SNOWFLAKE_ROLE_SP')
...
def main_func(folder,path,file_name,engine):
    ...
    if folder == 'Conciliaciones':
        ...
        df.to_sql(name='DESCONCILIACION_SINGLEPLAYER_GENERAL', con=engine, if_exists='append', index=False, chunksize=10000)
