...
class Extractor:
    ...
    def run(filename, **kwargs):
        ...
        if residuo > 0:
            ...
            df_D = df_D.append(df_l, ignore_index=True)
        df_D = pd.merge(df_D, df_B_A, left_on="importes", right_on="total", how="left")
        df_D = df_D.drop(['total','indice_y','file_name_y','indice_x','sum_deducciones','importe_bruto_A','monto_a_A','importes'], axis=1)
        df_D = df_D.fillna(0)
        df_D.columns = ['balance','importe','file_name','importe_valor','deducciones_valor','deducciones', 'iva_comision_contracargo', 'comision_contracargo']
        df_D = df_D[['importe','importe_valor','deducciones','deducciones_valor','balance','iva_comision_contracargo', 'comision_contracargo','file_name']]
