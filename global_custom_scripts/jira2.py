...
def extrac_histori(user,but):
    ...
    t = range(0,10)
    for i in t:
        for singleIssue in jira.search_issues(jql_str='project in (SDFPS)  AND issuetype = "Tarea - Simetrik" AND updated >= -2d  ORDER BY created DESC', expand='changelog',maxResults=100,startAt= 100*i):   
...
def fields_slas(issue):
    ...
    if len(issue['completedCycles']) > 0 :
        start_time = issue['completedCycles'][0]['breachTime']['jira']
        goal_time =  issue['completedCycles'][0]['startTime']['jira']
        hour_goal_time = issue['completedCycles'][0]['goalDuration']['millis']
        stop_time = issue['completedCycles'][0]['startTime']['jira']
        elapsedTime = issue['completedCycles'][0]['elapsedTime']['millis']
        remainingTime =   issue['completedCycles'][0]['remainingTime']['millis']
        status = 'completed'
    elif   le  > 0 :
        start_time = issue['ongoingCycle']['breachTime']['jira']
        goal_time = issue['ongoingCycle']['startTime']['jira']
