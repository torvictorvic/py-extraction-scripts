...
aiming = '/ml-sftp/xxxxxxx-Prod/reconciliations/xxxxxx/cards/mlb/conductor/'
name = 'VISA_225_'
s3_client= (boto3.client('s3'))
yesterday = datetime.today() - timedelta(1)
#pag = s3_client.get_paginator('list_objects_v2')
bucket_dest = "xxxxxxxxxxxxxx-cards-sources-xxxxxxxxxxxxxxxxxxx"
...
def lista_archivos(cliente, bucket, route):
    ...
    bucket -> Bucket where you want to read file names. example cards-sources-test
...
def run(filename, **kwargs ):
    print('Iniciando conductor', filename)
            ...
            for archive in sftp.listdir_attr(aiming):
                ...
                if name in archive.filename and yesterday < dt2_object:
                    ...
                    if not any( ts + ((archive.filename)).split('/')[-1] in s for s in key_list):
                        ...
                        print("El archivo ya esta :"+ route + archive.filename)                  
