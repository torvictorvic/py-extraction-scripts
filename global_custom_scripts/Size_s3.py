...
def list_files_bkp(client,bucket,date):
    ...
    df = pd.DataFrame(columns = ['BUCKET','FUENTE','ARCHIVO','FECHA_MODIFICACION','SIZE'])
    file_size = []
    for page in page_iterator:
        ...
        for  obj in cont:
            ...
            if obj['LastModified'].date() >= date[0].date() and   obj['LastModified'].date() <= date[1].date()  :
                split_text = str(obj['Key']).split('/')
                fs.append(str(bucket))
                fs.append(str(''.join(str('/'+str(e)) for e in split_text[:-1])))# Fuente
                fs.append(str(split_text[-1])) # Archivo
                fs.append(obj['LastModified'].strftime('%Y-%m-%d')) # Fecha de modificación
                fs.append(obj['Size'])
                df = pd.concat([df,pd.DataFrame([fs], 
                        columns=['BUCKET','FUENTE','ARCHIVO','FECHA_MODIFICACION','SIZE'])])
    return df
...
class Extractor:
    ...
    def run(filename, **kwargs ):
        ...
        time_fin = datetime.strptime(str(df['time_fin'][0]), '%Y-%m-%d %H:%M:%S' )  
        bucket = str(df['bucket'][0])
        date = [time_ini,time_fin]
