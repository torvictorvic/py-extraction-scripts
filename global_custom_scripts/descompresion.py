    ...
    def read(uri: str):
        ...
        if origin.scheme in ('s3', 's3a'):
            ...
            if ".rar" in uri:
                ...
                rar_file = RarFile(io.BytesIO(obj))
                for fn in rar_file.namelist():
                    s3.put_object( 
                        Body=rar_file.open(fn).read(),
                        Bucket=s3_url.bucket, 
                        Key='SFTP/PREPAGO/T2001/'+fn 
                    )                    
                df = None
                return df    
...
class T2001Parser:
    ...
    def parse(self):
        data = []
        manifest = list(filter(lambda x: x['record_type'] == self.type.value, RulesT2001.rules))[0]
        for line in self.io:
            item = {}
            if True:
...
class Extractor:
    ...
    def run(filename, **kwargs):
        ...
        for text_file in rar_file.infolist():
            try:
                formato = ["raw_body","tipo_resgitro","numero_tarjeta","marca","entidad_emisora","sucursal","numero_cuenta","tipo_socio","grupo_cuenta_corriente","tipo_transaccion","numero_comercio","nombre_fantasia_comercio","codigo_postal_comercio","codigo_autorizacion","ica_adquirente","codigo_movimiento","descripcion_movimiento","comprobante","tipo_plan","plan_cuotas","numero_cuota_vigente","fecha_operacion","hora_operacion","fecha_presentacion","fecha_cierre_cuenta_corriente","fecha_clearing","fecha_diferimiento","mcc","codigo_moneda","importe_modena_movimiento","codigo_moneda_original","importe_moneda_original","importe_interes","importe_iva","importe_total_movimiento","importe_descuento_financ_otrog","importe_descuento_financ_otrog_iva","importe_cuota","importe_interes_cuota","importe_iva_cuota","importe_total_cuota","DEBCRED","tipo_amortizacion","filler","TNA","TEA","tasa_intercambio","arancel_emisor","signo_iva_arancel","iva_arancel_emisor","signo_iva_arancel_2","motivo_mensaje","importe_compensacion","signo_importe_compensacion","importe_percep_rg4240","signo_percep_rg4240","tipo_producto","fecha_cierre_comercios","plan","moneda_compensacion","importe_autorizacion","signo_importe_autorizacion","moneda_autorizacion","importe_autorizacion_convertido","signo_importe_autorizacion_convertido","moneda_autorizacion_convertido","importe_ipm_d6","signo_importe_ipm_d6","moneda_ipm_d351","autoid","autocodi","cuenta_externa","request_id","filler_2","tipo_resgistro_h","marca_h","entidad_h","fecha_proceso_h","hora_proceso_h","archivo_h","descripcion_archivo_h","filler_h","tipo_registro_t","entidad_t","cant_reg_t","filler_t"]
                raw_data = rar_file.open(text_file.filename,'r').readlines()
                final = []
                for raw in raw_data:
                    final.append(raw.decode('latin1'))
