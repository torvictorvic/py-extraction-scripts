...
class BacenParser():
    def __init__(self, tabla, file_body, layouts, file_name='', sufix='cme',fil_size="True"):
        self.table = tabla
    ...
    def calculate_file_size(self):
        ...
        file = self.file_name.split(bucket)[-1]
        file_key = file.replace("/","",1)
    ...
    def parse(self):
        ...
        for key, val in self.layouts.items():
            ...
            # Agregar una nueva columna llamada 'FILE_SIZE_BYTES' al DataFrame con el valor del peso del archivo
            if self.fil_size=="True":
                file_size_bytes = self.calculate_file_size()
                file_size_bytes_1 = file_size_bytes / (1000000000)
                self.layouts[key]['df']['FILE_SIZE'] = round(file_size_bytes_1, 4)
        ...
        if self.table == "HL":
            ...
            self.layouts['RD']['df']['skt_extraction_rn'.upper()] = self.layouts['RD']['df'].index.values
...
class Cnab240Parser:
    ...
    def run(self, *args, **kwargs):
        ...
        for file_name in file_names:
            ...
            parser = BacenParser(tabla, decoded_file, layouts, file_name=file_name, sufix = '',fil_size=fil_siz)
