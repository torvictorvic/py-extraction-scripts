            ...
            if ".zip" in uri:
                ...
                for fn in zip_file.namelist():
                    ...
                    s3.put_object( 
                        ...
                        Key='AAA_Archivos/csv_test_MI_MO/'+fn 
                    )
                    print('='*50)
                    print(s3_url.bucket)
                    print('='*50)
                    print('AAA_Archivos/csv_test_MI_MO/'+fn)
...
class ExcelMt940Parser:
    ...
    general_description_code_mapping = {
        ...
        'CLC':
            {
                'DEVOLUCION' : {'dscr': 'CTC/341/', 'code': 'NTRF'},
                'PROVISION' : {'dscr': 'CTC/200/', 'code': 'NTRF'},
                'default': {'dscr': 'CTC/MSC/', 'code': 'NMSC' }
            },
        'CLS':
            {

                'TRANSFERENCIA' : {'dscr': 'CTC/MSC/', 'code': 'NTRF'},
                'default': {'dscr': 'CTC/MSC/', 'code': 'NTRF' }
            },
        'PEC':
            {
                'TRANSF.ORDINARIAS' : {'dscr': 'CTC/025/', 'code': 'NTRF'},
                'RECHAZ.TRNSF.ORDIN..' : {'dscr': 'CTC/081/', 'code': 'NTRF'},
                'default': {'dscr': 'CTC/450/', 'code': 'NTRF'}
            },
        'PEB':
            {
                'default': {'dscr': 'CTC/MSC/', 'code': 'NTRF' }
        ...
        'MX_BMEX':
            {
                'TRANS. INTERBANCARIA' : {'dscr': 'CTC/342/', 'code': 'NTRF'},
                'PAGO A TERCEROS' : {'dscr': 'CTC/322/', 'code': 'NTRF'},
                'DEPOSITO P/CONTRATO' : {'dscr': 'CTC/403/', 'code': 'NMSCP PROVEEDORES'},
                'REVERSO DE OPERACION' : {'dscr': 'CTC/306/', 'code': 'NMSC'},

                'DEPOSITO DE' : {'dscr': 'CTC/079/', 'code': 'NMSC'},
                'DEPOSITO S.B.C.' : {'dscr': 'CTC/635/', 'code': 'NCHKNONREF'},
                'DEVOL DOCTO S.B.C.' : {'dscr': 'CTC/312/', 'code': 'NCHK'},

                'default': {'dscr': 'CTC/MSC/', 'code': 'MSCNONREF' }
            }
        }
    ...
    def parse(df, file_name, headers_and_trailers, pais):
        ...
        tablas = {'BRA': 'br_citi_swift',
                  ...
                  'CLC': 'cl_citi_swift',
                  'CLS': 'cl_santander_swift',
                  'PEC': 'pe_citi_swift',
                  'PEB': 'pe_bbva_swift',
                  'MX_BMEX': 'mx_banamex_swift'}
        nombres = {'BRA': 'MPB_102352A_',
                  'COL': 'MPO_102559A_',
                  'UYU': 'DRU_102862A_',
                  'ARG': 'MLA_102216A_',
                  'AR_ONLINE':'MLA_102253A',
                  'CLC': 'MPC_102437A',
                  'CLS': 'MPC_102961A',
                  'PEC': 'DRP_102954A',
                  'PEB': 'DRP_102869A',
        ...
        referencias_de_banco = {'BRA': 'MCL1' + pd.Timestamp.now().strftime('%y%m%d') + '0637',
                  ...
                  'CLC': '9400120200700140',
                  'CLS': 'SIC0001123838',
                  'PEC': '9400120192770055',
                  'PEB': 'BGYY-MM-DD-00052',
                  'MX_BMEX': '9400120201430426'}
        for c in df.columns:
        ...
        swift_dict = {
            ...
            'TNX_TYPE_CODE': df['Descripción de la Transacción'].apply(lambda x: ExcelMt940Parser.defaul_code(x, pais)).tolist() ,
            'CLIENT_REFERENCE': df['Referencia del Cliente'].tolist(),
            'BANK_REFERENCE': df['Referencia del Banco'].tolist(),
            'DESCRIPTION': df['Descripción de la Transacción'].apply(lambda x: ExcelMt940Parser.defaul_description(x, pais)).tolist(),
            'FIELD_86': ':86:RPA_MANUAL',
    ...
    def run(filename, **kwargs):
        if '.xls' in filename.lower():
            ...
            for element in FILE_TO_CHOOSE:
                ...
                if element['country'] == "CH" and element['account_number'] == '102437A':
                    if 'CLC' == pais:
                        for extraction in element['extraction']:
                            if extraction['day'] == today:
                                extraction_method = extraction['method']
                ...
                if element['country'] == "CH" and element['account_number'] == '102961A':
                    if 'CLS' == pais:
                        for extraction in element['extraction']:
                            if extraction['day'] == today:
                                extraction_method = extraction['method']
                ...
                if element['country'] == "PE" and element['account_number'] == '102954A':
                    if 'PEC' == pais:
                        for extraction in element['extraction']:
                            if extraction['day'] == today:
                                extraction_method = extraction['method']
                ...
                if element['country'] == "PE" and element['account_number'] == '102869A':
                    if 'PEB' == pais:
                        for extraction in element['extraction']:
                            if extraction['day'] == today:
                                extraction_method = extraction['method']
                                if extraction_method == 'MANUAL':
                                    ...
                                    df = df.head(0)
                                    print('NO SE CARGO EL ARCHIVO POR SER DIA DE SWIFT', filename)
                if element['country'] == "MX" and element['account_number'] == '102602A':
                    if 'MX_BMEX' == pais:
                        for extraction in element['extraction']:
                            if extraction['day'] == today:
                                extraction_method = extraction['method']
            ...
            raise Exception('MO: El archivo cargado no tiene extension XLS')
...
class CSVMt940Parser:
    ...
    @staticmethod
    def run(filename, **kwargs):
        print('archivo a procesar: ', filename)
